<template>
    <header>
      <h2>TODO</h2>
      <label for="theme">
        <svg v-if="inDarkmode" xmlns="http://www.w3.org/2000/svg" width="26" height="26">
      ...
      <input id="theme" type="checkbox" @click="toggleDarkmode" v-model="inDarkmode">
    </header>
</template>

<script>
export default {
    emits:['toggle-darkmode'],
    data(){
      return {
          inDarkmode: JSON.parse(localStorage.getItem('darkmode')),
      }
    },
    methods:{
      toggleDarkmode(e){
        this.inDarkmode = e.target.checked;
        this.$emit('toggle-darkmode',this.inDarkmode)
      }
    }
}
</script>